    ...
    $.each(codeblocks, function(i, codeblock) {
      ...
      if (!$.data(codeblock, opts.data.codemirrorified)) {
        ...
        if (el.attr('highlight')) {
          var spec = getHighlightSpec(el.attr('highlight'));
          for (var lineNum in spec) {
            lineNum = parseInt(lineNum,10);
            var lineSpec = spec[lineNum];
            if (!lineSpec) continue;
            for (var specNum in lineSpec) {
              var specAtom = lineSpec[specNum];
              if (specAtom === '*') {
                editor.setLineClass(lineNum - 1, 'highlight');
              } else {
                var re = new RegExp('(' + specAtom + ')','g'),
                    cursor = editor.getSearchCursor(re,{line:lineNum - 1,ch:0}),
                    match;
                while (match = cursor.findNext()) {
                  if (cursor.to().line !== lineNum - 1) break;
                  editor.markText({line:lineNum-1,ch:cursor.from().ch},{line:lineNum-1,ch:cursor.to().ch},'highlight')
                }
              }
            }
          }
          for (i = 0; i < spec.global.length; i++) {
            var re = new RegExp('(' + spec.global[i] + ')','g'),
              cursor = editor.getSearchCursor(re,{line:0,ch:0}),
              match;
            while (match = cursor.findNext()) {
              editor.markText(
                {line:cursor.from().line,ch:cursor.from().ch},
                {line:cursor.from().line,ch:cursor.to().ch},
                'highlight'
              )
            }
          }
