        ...
        function _DoHeaders(text) {
            ...
            */
            ...
            text = text.replace(/(\#{1,6})(?:\.(\w+))?[ \t]+(.+?)[ \t]*\#*\n+/gm,
                function (wholeMatch, hashes, className, header) {
                    var h_level = hashes.length;
                    className = " class='" + (className || '') + "'";
                    return "<h" + h_level + className + ">" + _RunSpanGamut(header) + "</h" + h_level + ">\n\n";
                }
            );
          ...
          // attacklab: sentinel workarounds for lack of \A and \Z, safari\khtml bug
            ...
            text = text.replace(/(?:\n\n|^)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g,
                function (wholeMatch, m1, m2) {
                    var codeblock = m1;
                    var nextChar = m2;
                    var runnable = '',runMatch;

                    codeblock = _EncodeCode(_Outdent(codeblock));
                    if (runMatch = codeblock.match(/^\.runnable(?:\s(.*))?\s*$/m)) {
                      codeblock = codeblock.replace(/^\.runnable.*$/m,'');
                      runnable = ' runnable="true" ' + runMatch[1];
                    }
                    codeblock = _Detab(codeblock);
                    codeblock = codeblock.replace(/^\n+/g, ""); // trim leading newlines
                    codeblock = codeblock.replace(/\n+$/g, ""); // trim trailing whitespace

                    codeblock = "<pre><code class='code' " + runnable + " >" + codeblock + "</code></pre>";
